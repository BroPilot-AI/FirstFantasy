...
test.describe('Clinic Flow', () => {
  ...
  test('should display credits in clinic', async ({ gamePage }) => {
    const credits = await gamePage.page.locator('#clinic-scene.scene.active [data-testid="clinic-credits"]').textContent();
    expect(Number(credits)).toBeGreaterThan(0);
  });

  test('should heal party', async ({ gamePage }) => {
    await gamePage.page.locator('#clinic-scene.scene.active [data-testid="btn-heal"]').click();
    await gamePage.page.waitForTimeout(300);
    const dialogue = await gamePage.page.locator('#clinic-scene.scene.active [data-testid="clinic-dialogue"]').textContent();
    expect(dialogue).toContain('Good as new');
  });

  test('should leave clinic and return to town', async ({ gamePage }) => {
    await gamePage.page.locator('#clinic-scene.scene.active [data-testid="btn-leave-clinic"]').click();
    await gamePage.page.waitForTimeout(500);
